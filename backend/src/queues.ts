...
// Only create Bull queues if Redis is configured
export const userMonitor = connection ? new BullQueue("UserMonitor", connection) : null;

export const queueMonitor = connection ? new BullQueue("QueueMonitor", connection) : null;

export const messageQueue = connection ? new BullQueue("MessageQueue", connection, {
  limiter: {
...
export const scheduleMonitor = connection ? new BullQueue("ScheduleMonitor", connection) : null;
export const sendScheduledMessages = connection ? new BullQueue(
  "SendSacheduledMessages",
  connection
) : null;

export const campaignQueue = connection ? new BullQueue("CampaignQueue", connection) : null;
    ...
    if (campaign) {
      ...
      if (isArray(contacts)) {
        ...
        if (campaignQueue) {
          ...
          logger.warn("Campaign queue not available - Redis disabled");
        }
...
export async function startQueueProcess() {
  ...
  // Only setup queues if Redis is available
  if (!connection) {
